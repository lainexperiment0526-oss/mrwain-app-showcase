...
async function verifyPiAccessToken(token: string) {
  ...
  if (!r.ok || !data?.uid) { const err = new Error(data?.error || "Pi auth failed") as Error & { status?: number }; err.status = r.status || 401; throw err; }
...
async function resolveIncompletePayments(pi: PiClient, currentUid?: string, username = "", logStep?: ReturnType<typeof makeLogger>) {
  const incomplete = await pi.getIncompleteServerPayments().catch(async (error) => { await logStep?.("error", "Unable to read incomplete server payments", { error: extractErrorMessage(error) }); return []; });
  await logStep?.("info", "Checked incomplete server payments", { count: Array.isArray(incomplete) ? incomplete.length : 0 });
  ...
  for (const payment of ordered) {
    ...
    catch (error) { if (extractErrorMessage(error).includes("PI_WALLET_PRIVATE_SEED does not match")) throw error; await logStep?.("error", "Incomplete payment recovery failed", { payment_id: payment.identifier, error: extractErrorMessage(error) }); }
  }
  return results;
}
...
async function createPaymentWithRecovery(pi: PiClient, paymentData: Record<string, unknown>, username: string, logStep: ReturnType<typeof makeLogger>) {
  try {
    ...
    await logStep("warn", "Pi reports ongoing payment; starting recovery", { error: extractErrorMessage(error) });
    const recoveryResults = [];
    if (embeddedPayment?.identifier) {
      try { recoveryResults.push(await resolvePayment(pi, embeddedPayment, embeddedPayment.user_uid === String(paymentData.uid || "") ? username : "", { ignoreSeedMismatchWithoutTx: true }, logStep)); }
      catch (recoveryError) { await logStep("error", "Embedded ongoing payment recovery failed", { error: extractErrorMessage(recoveryError) }); throw recoveryError; }
    }
    ...
    try { return await pi.createPayment(paymentData); }
    catch (retryError) {
...
async function getAdminDashboard() {
  ...
  const logs = (txs || []).filter(t => String(t.status || "").startsWith("log_")).map(t => {
