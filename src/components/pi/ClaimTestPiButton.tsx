...
export default function ClaimTestPiButton({ onSuccess, className }: Props) {
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      const result = await claimTestnetPi(activeSession);
  ...
  return (
    <div className={className}>
      ...
      {lastTxid && (
        <p className="mt-1 text-center text-xs text-muted-foreground break-all">Success — txid: {lastTxid}</p>
      )}
    </div>
  );
}
